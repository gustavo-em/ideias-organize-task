...
def main() -> None:
    ...
    def android_vector(canvas: float, dp: float, share: float,
                       mono: bool = False) -> str:
        body = '\n'.join(
            f'    <path\n'
            ...
            for subs, colour in ((letter, WHITE if mono else INK), (rays, WHITE))
        )
        return (
            '<!-- Generated by scripts/generate-aluza-mark.py. Do not edit. -->\n'
    ...
    # The status bar icon. Android throws the colours away and paints the
    # alpha channel with its own tint, so the mark travels as one silhouette
    # instead of two paints — a full-colour launcher icon used here would
    # flatten into a solid square. Nothing masks it to a circle either, so it
    # fills more of its canvas than the splash does.
    NOTIFICATION_CANVAS = 24.0
    root = os.path.join(ROOT, 'android/app/src/main/res/drawable')
    open(os.path.join(root, 'ic_notification.xml'), 'w', encoding='utf8').write(
        android_vector(NOTIFICATION_CANVAS, NOTIFICATION_CANVAS, 0.92,
                       mono=True))
    print('android/drawable/ic_notification.xml')
